...
class SensorsRemoteListFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.getString("ARG_REMOTE_UNIT_ID")?.let {
            ...
            viewModel.setIdSensorRemoteForSearch(it)
        }
        ...
        viewModel.sensorRemoteListLiveData.observe(viewLifecycleOwner) { sensors ->
            sensorRemoteListAdapter.setList(sensors)

            binding?.emptyListTextView?.visibility =
                if (sensors.isNotEmpty()) View.INVISIBLE else View.VISIBLE
